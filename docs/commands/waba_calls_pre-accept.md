## waba calls pre-accept

Pre-accept an inbound call (early media setup)

### Synopsis

Optional but recommended: establishing WebRTC before accept avoids clipping the
first moments of audio. Respond within the 30-60s window after the Call Connect webhook.

```
waba calls pre-accept <call-id> [flags]
```

### Options

```
  -h, --help              help for pre-accept
