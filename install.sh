#!/bin/sh
# waba installer for macOS and Linux.
#
...
#   WABA_CLI_VERSION=v1.2.3   pin a version (default: the latest release)
...
REPO="jjuanrivvera/waba-cli"
BINARY="waba"
VERSION="${WABA_CLI_VERSION:-}"
...
if [ -z "$VERSION" ]; then
  ...
  [ -n "$VERSION" ] || die "could not determine the latest release; set WABA_CLI_VERSION"
fi
